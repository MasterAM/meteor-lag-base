...
Configurator = function (type, base, options) {
  ...
  this.initConfig();
  this.initCache();
  base.registerConfigurator(this);
};
...
_.extend(Configurator.prototype, {
  ...
  initDefaultConfigs: function(options) {
    this.defaultConfigs = _.extend(
      ...
      _.pick(options, _.union(this.customConfigNames, this.basicConfigNames))
    );
  },
  ...
  getActiveConfig: function(name) {
    var record = this.getCollection().findOne({
      type: 'config',
      name: name,
      level: this.type,
      isActive: true
  ...
  initConfig: function(){
    var configs = this._getSettings();
    var persist = this.base.getConfigOption('persist');
  ...
  /**
   * Attempts to get the meteor settings for this configurator
   * (normally specified in a JSON file).
   * @returns {Object} the settings, or an empty object if none are set
   * @private
   */
  _getSettings: function() {
    return this.base._resolve(Meteor, 'settings.lagConfig.' + this.type + "s") || {};
  },
  ...
  applySettings: function (config) {
    ...
    if (config.hasOwnProperty('exclude')) {
      ...
      configCollection.update({type: this.type}, {$set:{isExcluded: false}}, {multi: true});
