...
Lag = {
   ...
   */
  collectionName: 'alon_lag',

  /**
   * @property The collection that holds all of the settings.
   */
  collection: null,
  ...
  currentConfig: new ConfigCache('base'),
  ...
  getDelayForMethod: function(name) {
    ...
    method = this.getCollection().findOne({type: 'method', name: name});
    if (method) {
  ...
  isMethodBlocking: function(name) {
    var method = this.getCollection().findOne({type: 'method', name: name});
  ...
  addDelay: function(name, delay) {
    check(name, String);
    check(delay, Number);
    this.getCollection().upsert({type: 'method', name: name}, {$set: {delay: delay}});
  },
  ...
  clearDelay: function(name) {
    check(name, String);
    this.getCollection().upsert({type: 'method', name: name}, {$unset: {delay: 1}});
  },

  excludeMethod: function(name, doExclude) {
    this.getCollection().upsert({type: 'method', name: name}, {$set: {isExcluded: doExclude}});
  },

  setForceBlocking: function (name, forceBlocking) {
    this.getCollection().upsert({type: 'method', name: name}, {$set: {isBlocking: forceBlocking}});
  },
  ...
  initConfig: function(){
    ...
    } else if (typeof configs.disabled === "boolean") {
      // or if the `disabled` property is explicitly set
      this.applySettings({disabled: configs.disabled});
    }
    this.setInitialized();
  },
  ...
  applySettings: function (config) {
    ...
    _.each(this.basicConfigNames, function(propName) {
      if (config.hasOwnProperty(propName)) {
        configCollection.upsert({type: 'config', name: propName}, {$set: {value: config[propName]}});
      }
    });

    if (config.hasOwnProperty('exclude')) {
      // include all existing methods
      configCollection.update({type: 'method'}, {$set:{excluded: false}}, {multi: true});
    ...
    if (config.hasOwnProperty('methods')) {
      //unset all delays
      configCollection.update({type: 'method'}, {$unset: {delay: 1}}, {multi: true});
    ...
    if (config.hasOwnProperty('forceBlocking')) {
      //unset all
      configCollection.update({type: 'method'}, {$unset: {isBlocking: 1}}, {multi: true});
      //and set only the relevant ones
